...
class ViewController: UIViewController {
    ...
    // For TESTING
//    public var annotationToMove: Annotation?
    ...
    func startTimer() {
        if annotationManager.annotationToMove != nil {
            // For TESTING
            // Scheduling timer to Call the function "updateCounting" with the interval of 1 seconds
            timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(self.triggerCoordinteUpdate), userInfo: nil, repeats: true)

        }
    }
    var counter = 1
    var moveRight = true
    @objc func triggerCoordinteUpdate (){
//        print("Counter is \(counter), moveRight is \(moveRight)")
        
        if annotationManager.annotationToMove != nil {
            let currentLong = annotationManager.annotationToMove!.location.coordinate.longitude
            let currentLat = annotationManager.annotationToMove!.location.coordinate.latitude
            
            if moveRight == true {
                let newLong = currentLong + 0.000001
                let newLocation = CLLocation(latitude: currentLat, longitude: newLong)
                
                
                // Call a function in AnnotationManager that will update the anchor and then the location for annotationManager.annotationToMove
                annotationManager.testMoveAnnotation(annotation: annotationManager.annotationToMove!, updatedLocation: newLocation)
                if counter == 100 {
                    moveRight = false
                }
                else {
    ...
    @IBAction func didLongPress(_ recognizer: UILongPressGestureRecognizer) {
        ...
        if recognizer.state == .ended {
            ...
            // SWITCH FOR USING MY FUNCTION VS DIRECTIONS FUNCTION //
            
//          queryDirections(with: annotationLocation)
            placeTestAnnotations(with: annotationLocation)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let node = result?.node, let annotation = annotationManager.annotationsByNode[node] {
            ...
            // HERE you can respond to a user tapping a node in the AR scene!

            annotationManager.annotationToMove = annotation   // set the annotationManager.annotationToMove to the selected annotation
            startTimer()
            
        
            
            
//            annotationManager.removeAnnotation(annotation: annotation)  // Right now the annotation is removed when you tap on it.
        }
    }
    
    func placeTestAnnotations(with selectedLocation: CLLocation) {
        ...
        // Set's the AR orign location to current location - for Mapbox
        annotationManager.originLocation = currentLocation
        
        // An array of annotations to display
        var annotationsToAdd = [Annotation]()
        
        // Could create new points here
//        let stepLocation = CLLocation(latitude: coordinate.latitude, longitude: coordinate.longitude)
        
        // This should put a dot on the Mapbox map - currently doing it for currentLocation
        self.updateShapeCollectionFeature(&self.waypointShapeCollectionFeature, with: currentLocation, typeKey: "waypoint-type", typeAttribute: "big")
        // Testing this out with the selected location
        self.updateShapeCollectionFeature(&self.waypointShapeCollectionFeature, with: selectedLocation, typeKey: "waypoint-type", typeAttribute: "big")
        
        // Create an Annotation with the currentLocation and the optional UIImage called "arrived" - the 2D star icon
        let annotation = Annotation(location: currentLocation, calloutImage: UIImage(named: "arrived"))
        annotationsToAdd.append(annotation)
        
        // And again for the destination
        let otherAnnotation = Annotation(location: selectedLocation, calloutImage: UIImage(named: "arrived"))
        annotationsToAdd.append(otherAnnotation)
        ...
        let _ = directions.calculate(options) { (waypoints, routes, error) in
            ...
            if let route = routes?.first, let leg = route.legs.first {
                ...
                for step in leg.steps {
                    ...
                    // Add an AR node
                    let annotation = Annotation(location: stepLocation, calloutImage: self.calloutImage(for: step.description)) //step.description = things like "Head east", "Make a slight left", and "You have arrived at your destination, on the left"
                    print("self.calloutImage uses step.description which is: \(step.description)")
                    
                    print(self.calloutImage(for: step.description))
                    // When printed, looks like this:
                    // First icon: Optional(<UIImage: 0x1c02b0da0>, {502, 502})
                    // Steps: Optional(<UIImage: 0x1c02ab400>, {360, 360})
                    // Arrived point: Optional(<UIImage: 0x1c02ac5a0>, {104, 101})
...
extension ViewController: AnnotationManagerDelegate {
    ...
    func node(for annotation: Annotation) -> SCNNode? {
        print("The node(for annotation: Annotation) function inside ViewController.swift was just called an a new SCNNode created")
